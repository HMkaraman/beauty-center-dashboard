...
const NOTIFICATION_EVENTS: Record<string, NotificationEventConfig> = {
  ...
  appointment_reminder: {
    ...
    bodyTemplateAr: "{{clientName}} - {{service}} في {{date}} الساعة {{time}} ({{reminderLabel}})",
    bodyTemplateEn: "{{clientName}} - {{service}} on {{date}} at {{time}} ({{reminderLabel}})",
    targetRoles: ["owner", "admin", "manager", "receptionist"],
    targetAssigned: true,
    actionUrlTemplate: "/appointments/{{entityId}}",
  },
  appointment_no_show: {
    ...
    priority: "high",
    icon: "AlertTriangle",
    titleAr: "عميل لم يحضر",
    titleEn: "Client No-Show",
    bodyTemplateAr: "{{clientName}} - {{service}} في {{date}} الساعة {{time}}",
    bodyTemplateEn: "{{clientName}} - {{service}} on {{date}} at {{time}}",
    targetRoles: ["owner", "admin", "manager", "receptionist"],
    targetAssigned: true,
