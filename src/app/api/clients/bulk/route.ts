...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const updated = await db.update(clients).set({ status }).where(and(eq(clients.tenantId, session.user.tenantId), inArray(clients.id, ids))).returning({ id: clients.id });
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const deleted = await db
      .delete(clients)
      .where(and(eq(clients.tenantId, session.user.tenantId), inArray(clients.id, ids)))
