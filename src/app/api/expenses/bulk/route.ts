...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const updated = await db.update(expenses).set({ status }).where(and(eq(expenses.tenantId, session.user.tenantId), inArray(expenses.id, ids))).returning({ id: expenses.id });
    return success({ updated: updated.length });
  } catch (error) {
    console.error("PATCH /api/expenses/bulk error:", error);
    return serverError();
  }
}
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const deleted = await db
      .delete(expenses)
      .where(and(eq(expenses.tenantId, session.user.tenantId), inArray(expenses.id, ids)))
      .returning({ id: expenses.id });

    return success({ deleted: deleted.length });
  } catch (error) {
    console.error("DELETE /api/expenses/bulk error:", error);
    return serverError();
  }
}
