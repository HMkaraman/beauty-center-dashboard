...
import { doctors, doctorSchedules } from "@/db/schema";
...
export async function GET(_req: NextRequest, { params }: RouteParams) {
  try {
    ...
    const schedules = await db
      ...
      .where(
        and(
          eq(doctorSchedules.doctorId, id),
    ...
    console.error("GET /api/doctors/[id]/schedules error:", error);
    return serverError();
  }
}
...
export async function PUT(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    console.error("PUT /api/doctors/[id]/schedules error:", error);
    return serverError();
  }
}
