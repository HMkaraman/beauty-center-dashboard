...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const updated = await db.update(services).set({ status }).where(and(eq(services.tenantId, session.user.tenantId), inArray(services.id, ids))).returning({ id: services.id });
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const deleted = await db
      .delete(services)
      .where(and(eq(services.tenantId, session.user.tenantId), inArray(services.id, ids)))
      .returning({ id: services.id });

    return success({ deleted: deleted.length });
  } catch (error) {
    console.error("DELETE /api/services/bulk error:", error);
    return serverError();
  }
}
