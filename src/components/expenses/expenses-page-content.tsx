...
export function ExpensesPageContent() {
  ...
  const { items, searchQuery, setSearchQuery, deleteItem } = useExpensesStore();
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        {filtered.length === 0 ? (<p className="py-8 text-center text-sm text-muted-foreground">{tc("noResults")}</p>) : (
          <><ExpensesTable data={filtered} onEdit={handleEdit} onDelete={handleDelete} /><div className="space-y-3 md:hidden">{filtered.map((expense) => (<ExpenseCard key={expense.id} data={expense} onEdit={handleEdit} onDelete={handleDelete} />))}</div></>
        )}
      </div>
