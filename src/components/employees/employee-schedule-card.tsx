...
import { useEmployeeSchedules, useUpdateEmployeeSchedules } from "@/lib/hooks/use-employees";
...
const DEFAULT_SCHEDULE: ScheduleEntry[] = Array.from({ length: 7 }, (_, i) => ({
  ...
  startTime: "09:00",
  endTime: "17:00",
  isAvailable: i >= 1 && i <= 5 ? 1 : 0,
}));

export function EmployeeScheduleCard({ employeeId }: EmployeeScheduleCardProps) {
  const t = useTranslations("employees");
  const tDays = useTranslations("settings");
  const { data: schedulesData, isLoading } = useEmployeeSchedules(employeeId);
  const updateSchedules = useUpdateEmployeeSchedules();
