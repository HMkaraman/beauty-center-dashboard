      ...
      <table className="w-full text-sm">
        ...
        <tbody>{data.map((service) => (
          <tr key={service.id} className="border-b border-border last:border-0 hover:bg-secondary/20 transition-colors">
            ...
            <td className="px-4 py-3"><Link href={`/services/${service.id}`} className="flex items-center gap-3 hover:opacity-80"><Avatar size="sm"><AvatarFallback>{service.name.charAt(0)}</AvatarFallback></Avatar><p className="font-medium text-foreground hover:text-gold transition-colors">{service.name}</p></Link></td>
            ...
            <td className="px-4 py-3">
              <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon-xs"><MoreHorizontal className="h-4 w-4" /></Button></DropdownMenuTrigger>
                <DropdownMenuContent align="end"><DropdownMenuItem onClick={() => router.push(`/services/${service.id}/edit`)}>{t("edit")}</DropdownMenuItem><DropdownMenuItem onClick={() => onActivity?.(service)}>{t("activityLog")}</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem variant="destructive" onClick={() => onDelete?.(service.id)}>{t("delete")}</DropdownMenuItem></DropdownMenuContent>
              </DropdownMenu>
            </td>
          </tr>
        ))}</tbody>
