...
interface DoctorsSpecialtyChartProps {
  data: DonutSegment[];
}
...
export function DoctorsSpecialtyChart({ data }: DoctorsSpecialtyChartProps) {
  const t = useTranslations("doctors");
  const total = data.reduce((sum, d) => sum + d.value, 0);

  const translatedData = data.map((d) => ({
    ...d,
    name: t(d.name.replace("doctors.", "")),
  }));

  return (
    <ChartCard title={t("bySpecialty")}>
