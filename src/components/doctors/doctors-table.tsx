      ...
      <table className="w-full text-sm">
        ...
        <tbody>{data.map((doctor) => (
          <tr key={doctor.id} className="border-b border-border last:border-0 hover:bg-secondary/20 transition-colors">
            ...
            <td className="px-4 py-3"><DoctorStatusBadge status={doctor.status} /></td>
            <td className="px-4 py-3"><div className="flex items-center gap-1 font-english text-muted-foreground"><Star className="h-3.5 w-3.5 fill-yellow-400 text-yellow-400" />{doctor.rating > 0 ? doctor.rating : "—"}</div></td>
